import { readFileSync, writeFileSync, readdirSync, statSync } from 'fs-extra'
import { basename, join, dirname } from 'path'
import { Format } from './config'

function walk (dir: string, callback: (path: string) => void): void {
  const ls = readdirSync(dir)
  for (let i = 0; i < ls.length; i++) {
    const file = join(dir, ls[i])
    const namearr = basename(file).split('.')
    if (statSync(file).isFile() && namearr.length === 3 && namearr[1] === 'd' && namearr[2] === 'ts') {
      callback(file)
    } else if (statSync(file).isDirectory()) {
      walk(file, callback)
    }
  }
}

export interface DeclarationInfo {
  needWrap: { [key: string]: { ns: string; code: string } }
  users: { [key: string]: { code: string } }
}

function collectInfo (outDir: string): DeclarationInfo {
  const info: DeclarationInfo = {
    needWrap: {},
    users: {}
  }
  walk(outDir, (dtspath) => {
    const dirpath = dirname(dtspath)
    const code = readFileSync(dtspath, 'utf8')
    const re = new RegExp('import \\* as (\\S+) from [\'"](\\.\\S*\\/\\S+)[\'"]', 'g')
    let arr = re.exec(code)
    while (arr !== null) {
      const targetDts = join(dirpath, arr[2] + '.d.ts')
      if (info.needWrap[targetDts] === undefined) {
        const targetDtsCode = readFileSync(targetDts, 'utf8')
        info.needWrap[targetDts] = {
          ns: arr[1],
          code: targetDtsCode
        }
      }
      if (info.users[dtspath] === undefined) {
        info.users[dtspath] = { code }
      }
      arr = re.exec(code)
    }
  })

  return info
}

export function applyChange (outDir: string): DeclarationInfo {
  const info = collectInfo(outDir)
  const { needWrap: dts, users: user } = info
  for (const key in dts) {
    const originalCode = dts[key].code/*  = readFileSync(key, 'utf8') */
    let lines = originalCode.split(/\r?\n/)
    let lineNumber = 0
    for (let i = 0; i < lines.length; i++) {
      if (/^(export|declare) /.test(lines[i].trimLeft())) {
        lineNumber = i
        break
      }
    }
    if (lineNumber !== 0) {
      for (let i = lineNumber; i >= 0; i--) {
        if (/^\/\*\*/.test(lines[i].trimLeft())) {
          lineNumber = i
          break
        }
      }
    }

    let code = dts[key].code.replace(/declare /g, '').replace(/export /g, '')
    if (code.includes('//# sourceMappingURL=')) {
      code = code.replace(/(\/\/# sourceMappingURL=(.*)(\r?\n)?)/g, `}\nexport default ${dts[key].ns};\n$1\n`)
    } else {
      code = code + `\n}\nexport default ${dts[key].ns};`
    }
    lines = code.split(/\r?\n/)
    lines.splice(lineNumber, 0, `declare namespace ${dts[key].ns} {`)
    code = lines.join('\n')
    writeFileSync(key, code, 'utf8')
  }

  for (const key in user) {
    // user[key].code = readFileSync(key, 'utf8')
    let code = user[key].code
    for (const d in dts) {
      code = code.replace(new RegExp(`import \\* as (\\S+) from (['"]\\S+\\/${basename(d).split('.')[0]}['"])`, 'g'), 'import $1 from $2')
    }
    writeFileSync(key, code, 'utf8')
  }

  return info
}

export function revertChange (info: DeclarationInfo): void {
  for (const key in info.needWrap) {
    writeFileSync(key, info.needWrap[key].code, 'utf8')
  }
  for (const key in info.users) {
    writeFileSync(key, info.users[key].code, 'utf8')
  }
}

export function resolveDeclarationFile (dtsPath: string, ns: string, format: Format = 'umd'): void {
  const dts = readFileSync(dtsPath, 'utf8')
  if (format === 'umd') {
    const umddts = `${dts}\nexport as namespace ${ns}\n`
    writeFileSync(dtsPath, umddts, 'utf8')
  } else if (format === 'cjs') {
    let cjsDts = dts.replace(/declare\s/g, '')
    cjsDts = cjsDts.replace(/export default (\S+);/g, 'export { $1 as default }')
    cjsDts = `declare namespace ${ns} {\n${cjsDts}`
    cjsDts += `\n}\nexport = ${ns}\n`
    writeFileSync(dtsPath, cjsDts, 'utf8')
  } else if (format === 'iife') {
    let globalDts = dts.replace(/declare\s/g, '')
    globalDts = globalDts.replace(/export default (\S+);/g, 'export { $1 as default }')
    globalDts = `declare namespace ${ns} {\n${globalDts}`
    globalDts += '\n}\n'
    writeFileSync(dtsPath, globalDts, 'utf8')
  } else {
    throw new TypeError('Format: umd | cjs | iife')
  }
}
